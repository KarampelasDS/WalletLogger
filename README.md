...
- **Three transaction types** — Income, Expense, and account-to-account Transfers, each with date/time, note, emoji-tagged category, and account selection via a custom numeric keypad and option pickers.
- **True multi-currency** — add any number of currencies, store a per-currency exchange rate to your main currency, and record transactions in their native currency while balances and reports stay in the main unit.
